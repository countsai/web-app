...
export default function ReferralLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch("/api/referrals/login", {
        method: "POST",
        ...
        <div className="w-full max-w-md space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-sm font-medium" style={{ color: "#5f7679" }}>
              Track your referrals, wallet balance, and payouts.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 bg-white rounded-2xl p-6 sm:p-8" style={{ border: "1px solid #dbe9eb" }}>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input type={showPw ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)}
                  placeholder="Your password" className={`${inputBase} pr-10`} style={inputStyle}
            ...
            <p className="text-center text-[11px] font-medium" style={{ color: "#85a0a4" }}>
              ...
              <Link href="/referrals/signup" className="font-black hover:underline" style={{ color: "#079DB3" }}>Sign up</Link>
            </p>
          </form>
        </div>
      </main>
